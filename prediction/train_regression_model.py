...
class ResNet(nn.Module):
    def __init__(self, out_size=1):
        ...
        ckpt = torch.load("prediction/model_003epochs.pth", map_location=torch.device('cpu'))['model_state_dict'] # resnet50
        new_state_dict = OrderedDict()
        for k, v in ckpt.items():
            name = k.replace("resnet.", "")
            new_state_dict[name] = v
        new_state_dict['fc.weight'] = torch.zeros([1000, 2048]) # resnet50
        new_state_dict['fc.bias'] = torch.zeros([1000]) # resnet50
